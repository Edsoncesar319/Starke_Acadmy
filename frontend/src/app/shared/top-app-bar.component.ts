    ...
    >
      <div class="flex min-w-0 items-center gap-2 sm:gap-3">
        <button
          ...
          class="btn-touch shrink-0 rounded-lg border border-gold-500/30 text-gold-300 transition hover:bg-gold-500/10 lg:hidden"
        ...
        <button
          type="button"
          (click)="toggleNav.emit()"
          class="btn-touch hidden shrink-0 rounded-lg border border-gold-500/30 text-gold-300 transition hover:bg-gold-500/10 lg:inline-flex"
          [attr.aria-label]="navCollapsed ? 'Expandir menu lateral' : 'Recolher menu lateral'"
          [title]="navCollapsed ? 'Expandir menu' : 'Recolher menu'"
        >
          <mat-icon
            class="material-symbols-rounded transition-transform duration-300 motion-reduce:transition-none"
      ...
      <div class="flex min-w-0 items-center justify-between gap-2 sm:justify-end sm:gap-3">
        ...
        <button
          type="button"
          (click)="logout.emit()"
          class="btn-outline shrink-0 gap-1.5 px-2 sm:px-3"
          aria-label="Sair da conta"
        >
          <mat-icon class="material-symbols-rounded !h-4 !w-4 !text-base">logout</mat-icon>
